class PainelEstatisticas {
  ...
  buildLocalDataset() {
    ...
    const brasil = {
      AC: {
        cidade: "Rio Branco",
        municipios: ["Rio Branco", "Cruzeiro do Sul"],
      },
      ...
      GO: {
        cidade: "Goiânia",
        municipios: ["Goiânia", "Aparecida de Goiânia"],
      },
      ...
      PE: {
        cidade: "Recife",
        municipios: ["Recife", "Jaboatão dos Guararapes"],
      },
      PI: { cidade: "Teresina", municipios: ["Teresina", "Parnaíba"] },
      RJ: {
        cidade: "Rio de Janeiro",
        municipios: ["Rio de Janeiro", "São Gonçalo"],
      },
      RN: { cidade: "Natal", municipios: ["Natal", "Mossoró"] },
      RS: {
        cidade: "Porto Alegre",
        municipios: ["Porto Alegre", "Caxias do Sul"],
      },
      ...
      SE: {
        cidade: "Aracaju",
        municipios: ["Aracaju", "Nossa Senhora do Socorro"],
      },
      ...
      ufs.forEach((uf, idxUf) => {
        ...
        estado.municipios.forEach((municipio, idxMun) => {
          tipos.forEach((tipo, idxTipo) => {
            const base =
              72 + idxAno * 10 + (idxUf % 11) * 6 + idxMun * 8 + idxTipo * 4;
            const casos = Math.max(8, Math.round(base * pesoTipo[tipo]));
            const denuncias = Math.max(
              4,
              Math.round(casos * (0.7 + idxTipo * 0.04)),
            );
  ...
  renderFilterControls() {
    ...
    const controls = [
      this.createSelectControl("Ano", "ano", [
        "todos",
        ...this.uniqueValues("ano"),
      ]),
      this.createSelectControl("Estado (UF)", "uf", [
        "todos",
        ...this.uniqueValues("uf"),
      ]),
      this.createSelectControl("Cidade", "cidade", [
        "todos",
        ...this.uniqueValues("cidade"),
      ]),
      this.createSelectControl("Tipo de Violência", "tipo", [
        "todos",
        ...this.uniqueValues("tipo"),
      ]),
  ...
  createSelectControl(labelText, key, options) {
    ...
    options.forEach((optionValue) => {
      ...
      option.textContent =
        optionValue === "todos" ? "Todos" : String(optionValue);
  ...
  syncCidadeOptions() {
    ...
    const base = this.rawData.filter((row) => {
      ...
      if (municipio && !row.municipio.toLowerCase().includes(municipio))
        return false;
      return true;
    });
    ...
    cidadeSelect.innerHTML = cities
      .map(
        (city) =>
          `<option value="${city}">${city === "todos" ? "Todos" : city}</option>`,
      )
      .join("");
  ...
  renderRanking(root, items, subtitle) {
    if (!items.length) {
      root.innerHTML =
        '<p class="tabela-vazia">Sem dados para os filtros selecionados.</p>';
      return;
    }
  ...
  renderTable(rowsPage, totalItems) {
    ...
    if (!rowsPage.length) {
      this.rootTableBody.innerHTML =
        '<tr><td colspan="7" class="tabela-vazia">Nenhum registro encontrado para os filtros aplicados.</td></tr>';
      return;
    }

    this.rootTableBody.innerHTML = rowsPage
      .map(
        (row) => `
            <tr>
                <td>${row.ano}</td>
  ...
  renderPagination(meta) {
    ...
    const prevBtn = this.rootPagination.querySelector(
      '[data-page-action="prev"]',
    );
    const nextBtn = this.rootPagination.querySelector(
    ...
    if (prevBtn) {
      prevBtn.addEventListener("click", () => {
        this.state.pagination.page = Math.max(
          1,
          this.state.pagination.page - 1,
        );
        this.refresh();
    ...
    if (nextBtn) {
      nextBtn.addEventListener("click", () => {
        this.state.pagination.page = Math.min(
          totalPages,
          this.state.pagination.page + 1,
        );
        this.refresh();
  ...
  refresh() {
    ...
    this.renderRanking(
      this.rootTipos,
      this.groupSum(filtered, "tipo", "casos"),
      "Casos registrados",
    );
    this.renderRanking(
      this.rootEstados,
      this.groupSum(filtered, "uf", "casos"),
      "Casos por estado",
    );
    this.renderRanking(
      this.rootCidades,
      this.groupSum(filtered, "cidade", "denuncias"),
